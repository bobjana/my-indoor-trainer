...
// Power Zone Utilities

export class PowerZones {
    // Calculate power zone (1-5) based on power and FTP
    static getZone(power, ftp = 200) {
        if (power === 0) return 0;
        if (!ftp || ftp === 0) ftp = 200;

        const percentage = (power / ftp) * 100;

        if (percentage < 55) return 1;       // Zone 1: Recovery (<55% FTP)
        if (percentage < 75) return 2;       // Zone 2: Endurance (55-75% FTP)
        if (percentage < 90) return 3;       // Zone 3: Tempo (75-90% FTP)
        if (percentage <= 105) return 4;     // Zone 4: Threshold (90-105% FTP)
        return 5;                            // Zone 5: VO2 Max (>105% FTP)
    }

    // Get color for a power zone
    static getZoneColor(zone) {
        const colors = {
            0: '#666666',  // No power data - gray
            1: '#4ade80',  // Zone 1 - green
            2: '#22d3ee',  // Zone 2 - cyan
            3: '#fbbf24',  // Zone 3 - yellow/amber
            4: '#fb923c',  // Zone 4 - orange
    ...
    // Get color directly from power value
    static getPowerColor(power, ftp = 200) {
        const zone = this.getZone(power, ftp);
        return this.getZoneColor(zone);
    }
}
